...
<div class="bas_news_list_head">
	<h1><?$APPLICATION->ShowTitle(false)?></h1>

<?	$arAvailableSort = array("ACTIVE_FROM", "SHOW_COUNTER");
	$arAvailableOrder = array("ASC", "DESC");

	if (isset($_GET['sort'])) $sort = $_REQUEST['sort'];
	else $sort = $_SESSION['bas_news_sort'];

	if (isset($_GET['order'])) $order = $_REQUEST['order'];
	else $order = $_SESSION['bas_news_order'];

	if (!in_array($sort, $arAvailableSort)) $sort = 'ACTIVE_FROM';
	if (!in_array($order, $arAvailableOrder)) $order = 'DESC';

	$_SESSION['bas_news_sort'] = $sort;
	$_SESSION['bas_news_order'] = $order;

	$toUrl = ($order === 'ASC') ? "DESC" : "ASC"?>

	<div class="bas_news_sort">
