...
if ($PREVIEW_WIDTH <= 0) $PREVIEW_WIDTH = 75;

$PREVIEW_HEIGHT = intval($arParams["PREVIEW_HEIGHT"]);

if ($PREVIEW_HEIGHT <= 0) $PREVIEW_HEIGHT = 75;
...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	if (is_array($arParams["PRICE_CODE"])) $arResult["PRICES"] = CIBlockPriceTools::GetCatalogPrices(0, $arParams["PRICE_CODE"]);
	else $arResult["PRICES"] = array();
	...
	{
		$arElement["PRICE"] = number_format($arElement['PROPERTY_PRICE_VALUE'], 0, '', ' ') . ' руб.';
