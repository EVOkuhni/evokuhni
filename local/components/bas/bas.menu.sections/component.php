...
// дописанно
if (isset($arParams["SECTION_ID"]) && (intval($arParams["SECTION_ID"]) > 0)) $arParams["SECTION_ID"] = intval($arParams["SECTION_ID"]);
else $arParams["SECTION_ID"] = false;
...
if($this->StartResultCache())
{
	...
	else
	{
		...
		if ($arParams["SECTION_ID"]) $arFilter["SECTION_ID"] = intval($arParams["SECTION_ID"]); // дописанно
		...
		while($arSection = $rsSections->GetNext())
		{
			$arResult["SECTIONS"][] = array(
				"ID" => $arSection["ID"],
				"DEPTH_LEVEL" => $arParams["SECTION_ID"] ? $arSection["DEPTH_LEVEL"] - 1 : $arSection["DEPTH_LEVEL"], // дописанно
				"~NAME" => $arSection["~NAME"],
